...
export default function SessionProposalModal() {
  ...
  const supportedNamespaces = useMemo(() => {
    // eip155
    const eip155Chains = ["eip155:1", "eip155:5"]; //Object.keys(EIP155_CHAINS);
    ...
    return {
      eip155: {
        ...
        accounts: eip155Chains.map((chain) => `${chain}:${tbaAddress}`).flat(),
      },
    };
  }, []) as any;
  console.log("supportedNamespaces", supportedNamespaces, tbaAddress);
            ...
            supportedChains.map((chain, i) => {
              return (
                <Row key={i}>
                  <ChainAddressMini
                    key={i}
                    address={getAddress(chain?.namespace)}
                  />
                </Row>
              );
            })}
